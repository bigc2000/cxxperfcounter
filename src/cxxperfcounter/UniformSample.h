...
public:
  ...
  UniformSample(int reservoirSize)noexcept : cur(0), count(0), maxSize(reservoirSize) {
    values.resize(maxSize);
  }

  UniformSample(const UniformSample &src)noexcept : cur(src.cur), count(src.count), maxSize(maxSize), values(src.values) {
    values.resize(maxSize);
  }

  UniformSample(UniformSample &&src)noexcept : cur(src.cur), count(src.count), maxSize(maxSize) {
    values = src.values;
    values.resize(maxSize);
  }
